<?php
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/session.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/model/config.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/download.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/xpath.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/simple_html_dom.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/model/Model.php';
class WebScraperController {
	...
	public function getData() {
		if (isset ( $_GET ['page'] )) {
			...
			$temp =implode("",Session::get_base_url ());
			...
			if ($data != '') {				
				...
				//if(preg_match($checkstr,$dsalary)){
					//$dfromsalary =$dsalary;
					//$dtosalary =' ';
				//}else{
					//$dfromsalary =substr($dsalary,0,strlen($dsalary)/2);
					//$dtosalary =substr($dsalary,strlen($dsalary)/2,strlen($dsalary));
				//}
				...
				if ($djob != '') {
					...
					if (! $this->jobModel->isExist ( $source )) {
						$ret = $this->jobModel->AddNewJob($djob, $dlocation, $dsalary, $ddes, $dcompany, $dtag, $drequirement, $dbenifit, $dexpired, $source, $dlogo);
					} else {
						echo "Exist";
					}
				}
