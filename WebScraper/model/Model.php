...
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/connection.php';
require_once $_SERVER ["DOCUMENT_ROOT"] . '/WebScraper/lib/Encoding.php';
...
class JobModel extends Model {
	...
	public function AddNewJob($JobName, $Location, $Salary, $Description, $Company, $Tags, $Requirement, $Benifit, $Expired, $Source, $dlogo) {
				
		$JobName = addslashes($JobName);
		$Location = addslashes($Location);
		//$FromSalary =addslashes($FromSalary);
		//$ToSalary =addslashes($ToSalary);
		...
		$sql = "INSERT INTO `job`(`JobName`,`Location`, `Salary`, `Description`, `Company`, `Tags`, `Requirement`, `Benifit`, `Expired`, `Source`, `Logo`)". " VALUES (N'$JobName',N'$Location',N'$Salary',N'$Description',N'$Company',N'$Tags',N'$Requirement',N'$Benifit',N'$Expired',N'$Source','$dlogo')";		
...
class XPathModel extends Model {
	...
	public function save($home_url, $base_url, $xpath_code, $login_url, $login_data, $xpath, $company_xpath, $location_xpath, $description_xpath, $salary_xpath, $requirement_xpath, $benifit_xpath, $expired_xpath, $tags_xpath) {
		...
		$this->connection->connect ();
		$ret = $this->connection->write ( "INSERT INTO `xpath`(`home_url`, `base_url`, `xpath_code`, `login_url`, `login_data`, `xpath`, `company_xpath`, `location_xpath`, `description_xpath`, `salary_xpath`, `requirement_xpath`, `benifit_xpath`, `expired_xpath`, `tags_xpath`) VALUES ('$home_url','$base_url','$xpath_code','$login_url','$login_data','$xpath','$company_xpath','$location_xpath','$description_xpath','$salary_xpath','$requirement_xpath','$benifit_xpath', '$expired_xpath', '$tags_xpath')" );
		$this->connection->close ();
		return $ret;
	}
	public function get($home_url) {
		$this->connection->connect ();
		$data = $this->connection->read ( "SELECT  * from xpath where home_url='" . $home_url . "'" );
		$this->connection->close ();
		return $data;
	}
